...
public class NearbyWords implements SpellingSuggest {
		...
		for(int index = 0; index <= s.length(); index++){
			...
			for(int charCode = (int)'a'; charCode <= (int)'z'; charCode++) {
				if(index==s.length())
					...
					t = s.substring(0, index)+(char)charCode+s.substring(index);
				if(!currentList.contains(t) && (!wordsOnly||dict.isWord(t))) {
					currentList.add(t);
				}
			}
		}
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while(!queue.isEmpty() && retList.size()<numSuggestions && j<THRESHOLD)
		{
			j++;
			curr=queue.get(0);
			queue.remove(0);
			neighbour=distanceOne(curr,false);
			for(int i=0;i<neighbour.size() && retList.size()<numSuggestions;i++)
			{
				if(!visited.contains(neighbour.get(i)))
				{
					visited.add(neighbour.get(i));
					queue.add(neighbour.get(i));
					if(dict.isWord(neighbour.get(i)))
						retList.add(neighbour.get(i));
				}
			}
			neighbour.clear();
		}
