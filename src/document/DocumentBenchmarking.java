...
public class DocumentBenchmarking {
	...
	public static void main(String [] args) {
				...
				numToCheck += increment)
		{
			...
			long st,ft;
			double ans;
			String ret = getStringFromFile(textfile,numToCheck);
			st = System.nanoTime();
			for(int i=0;i<trials;i++){
				ans=new BasicDocument(ret).getFleschScore();
			}
			ft = System.nanoTime();
			System.out.print((ft - st)/1000000000+"\t");
			st = System.nanoTime();
			for(int i=0;i<trials;i++){
				ans=new EfficientDocument(ret).getFleschScore();
			}
			ft = System.nanoTime();
			System.out.print((ft - st)/1000000000+"\n");
